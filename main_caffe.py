...
parser.add_argument('--data', metavar='DIR', default='/media/shared-corpus/imagenet-mxnet/', 
...
def main_worker(ngpus_per_node, args):
    ...
    from alexnet_my import Alexnet
    model = Alexnet()
...
##
#    the_model = resnet18()
#    state_dict = torch.load('res18_regular_8.pth')
#    for key1 in the_model.state_dict().keys():
#        if key1 not in state_dict.keys():
#            state_dict[key1] = the_model.state_dict()[key1]
...
#                  .format(args.resume, checkpoint['epoch']))
    ...
    if args.evaluate:  
        ...
        for epoch in range(1):
            optimizer_admm = SGD_caffe(model.parameters(), args.lr,weight_decay=5e-4,momentum=0.9)        
            optimizer_admm.param_groups[0]['lr'] = 1e-4
            train(train_loader, model, criterion, optimizer_admm, epoch, args,None,None,False,0)
            acc1 = validate(val_loader, model, criterion, args)
            torch.save(model.cpu().state_dict(),'Alex_true_base.pth')
    ...
    optimizer_admm = CaffeSGD(model.parameters(), args.lr,weight_decay=5e-4,momentum=0.9)        
    ...
    conf = read_config('/home/ww/alex-206/conf/alex_x18.conf')
    sparse_param = admm_sparse.ADMMParameter(conf)
    sparse_param._PruneModel(model)
    print_prune(model) 
    #torch.save(model.cpu().state_dict(),'Alexnet_18x.pth')
    optimizer = SGD(model.parameters(), args.lr,
                                  momentum=args.momentum,
                                  weight_decay=args.weight_decay)   
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i,(input,target) in enumerate(val_loader):
           ...
           # input = data[0]["data"].cuda(non_blocking=True)
            input = torch.round(input * 255).cuda(non_blocking=True)
           # target = data[0]["label"].squeeze().long().cuda(non_blocking=True)
