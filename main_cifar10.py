...
#                  .format(args.resume, checkpoint['epoch']))
    ...
    optimizer_pre = torch.optim.Adam(model.parameters(), args.lr,weight_decay=5e-4)
    
    for epoch in range(100): 

        optimizer_admm.param_groups[0]['lr'] = 1e-1
        if epoch >= 30:
            optimizer_admm.param_groups[0]['lr'] = 1e-2
        if epoch >= 60:
            optimizer_admm.param_groups[0]['lr'] = 1e-3

        train(train_loader, model, criterion, optimizer_admm, epoch,args,None,None,False)
