...
def main_worker(ngpus_per_node, args):
    ...
    #from res50_my import res50
    #model = res50()
    #model.load_state_dict(torch.load('res50_baseline.pth'))
...
#  
    the_model = res50()
    state_dict = torch.load('Res50_x18.pth')
...
#                  .format(args.resume, checkpoint['epoch']))
    ...
    print_prune(model) 
    #torch.save(model.cpu().state_dict(),'res_18x.pth')
    #model.load_state_dict(torch.load('res_18x.pth'))
    optimizer = SGD(model.parameters(), args.lr,
                                  momentum=args.momentum,
                                  weight_decay=args.weight_decay)  
...
def bn_freeze(model): # map bn weight to conv
